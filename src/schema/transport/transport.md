...
> `httpSettings`: [HttpUpgradeObject](./transports/httpupgrade.md)

HTTPUpgrade configuration for the data stream. Only valid when `network` is `httpupgrade`.

> `grpcSettings`: [gRPCConfigObject](./transports/grpc.md)
...
> `hysteria2Settings`: [Hysteria2Object](./transports/hysteria.md)
...
Transport security configuration. Only valid when `security` is not `"none"`.

> `tlsSettings`: [TLSObject](./transports/tls.md)

TLS configuration. TLS is provided by Go. In normal cases the negotiation result will be TLS 1.3.
