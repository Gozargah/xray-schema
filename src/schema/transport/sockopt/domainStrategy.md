...
Trigger condition: the proxy server address is a domain name (`proxy.com`), and the built-in DNS server is in non-Local mode. Before Xray establishes a TCP connection to `proxy.com`, it queries `proxy.com` through the built-in DNS server. If bad routing rules cause that DNS query to be proxied through `proxy.com`, the connection waits for the query result while the query cannot finish until the connection is established.
