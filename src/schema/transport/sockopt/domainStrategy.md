...
Trigger condition: the proxy server address is a domain name (`proxy.com`), and the built-in DNS server is in non-Local mode. Before Xray establishes a TCP connection to `proxy.com`, it queries `proxy.com` through the built-in DNS server. The built-in DNS server connects to `dns.com` and sends a query to obtain the IP of `proxy.com`. Bad routing rules cause that request to be proxied through `proxy.com`, so Xray tries to establish another TCP connection to `proxy.com` and again queries `proxy.com` through the built-in DNS server. The built-in DNS server reuses the pending connection and sends the new query, but that connection is waiting for the new query result while the new query cannot finish until the connection is fully established. Direct connections through Freedom can have the same problem: if connecting to a DNS server requires resolving its own domain name through that same server, a circular dependency is created.
